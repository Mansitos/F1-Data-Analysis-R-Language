...
```{r,include = !knit_relazione, warning=FALSE,message=FALSE, echo=!knit_relazione,fig.width=10, fig.height=6}
...
```{r,include = !knit_relazione, warning=FALSE,message=FALSE, echo=!knit_relazione}
...
```{r,include = !knit_relazione, warning=FALSE,echo=!knit_relazione,message=FALSE,fig.width=10, fig.height=6}
...
```{r,include = !knit_relazione, warning=FALSE,echo=!knit_relazione,message=FALSE,fig.width=10, fig.height=5}
...
In this presentation we will analize data about [Formula One](https://www.formula1.com/) history. We will explore data relating drivers, constructors, countries, circuits etc. involved in the history of this motorsport. We will try to find out which were the most dominant drivers and constructors in the history of Formula 1, evaluating their results taking into consideration more sophisticated criteria than the most classic ones. Statistics relating to the F1 itself, most iconic drivers, constructors and countries will also be shown. All the used data is available [here](https://www.kaggle.com/rohanrao/formula-1-world-championship-1950-2020) and consists of 13 csv files. Sourcecode is available [here](https://github.com/Mansitos/Progetto-DataScience-UNI-F1-Data-Analysis-R-Language)
...
Formula One cars are considered the fastest regulated road-course racing cars in the world, haracterized by very high cornering speeds, achieved through the generation of large amounts of aerodynamic downforce. Most modern F1 cars can achieve peaks of 6.5 lateral g while cornering and approximately top speeds of 360km/h. Tracion control and other driving aids have been banned since 2008.
...
```{r, out.width = "1000px",echo = !knit_relazione,fig.align='center'}
knitr::include_graphics("imgs\\stats_img.jpg")
```

### First F1 winner
Giuseppe Farina (also known as Giuseppe Antonio “Nino” Farina) won the first F1 race and the first F1 Driver Championship. Farina drove for Ferrari, Alfa Romeo and Lancia. Let’s see what the data can tell us about this iconic driver:
...
After the win of the first championship by Farina, the 1950s were dominated by two iconic drivers, Alberto Ascari and Juan Manuel Fangio:


Fangio’s records remained unbeaten for over 30 years. 1950s are remembered as the deadliest decade in F1, with 15 casualties. Helmetds became mandatory 2 years after the birth of F1, in 1952. Seatbelts became mandatory in 1972.
...
*“Jim Clarck was everything I aspired to be, as a racing driver and as a man” - Sir Jackie Stewart*

### The Flying Scot
Clark’s 25 wins record was broken by his greatest admirer: Sir John Jackie Stewart.

### The battle between teammates
It took 14 years to break Stewart’s winning record; a new one began, and the progenitor was Alain Prost.
...
# Who is the most dominant driver of all time?
Which driver has dominated the circuits during the history of Formula One? It's difficult to tell, but data can help us! Often drivers are compared taking into consideration only simple metrics, such as: *races won*, *world titles* and so on. Surely those metrics reflect the skill of the driver, however they are superficial evaluation criteria. For example, consider the case in which the driver *a* has won 100 races out of 500 and driver *b* has won 70 races out of 120. Would you still say that driver *a* is better than driver *b* just because he has won more races? The data collected during these 71 years can help us build a more advanced evaluation system, based on multiple parameters, which will allow us to evaluate drivers in a more transparent and objective way. Let's start by showing some basic drivers summaries which we will use later for evaluating the performance of drivers.
...
A significant metric in evaluating a driver's skill is the *amount of points* cumulated during their career. However, there is a  problem: *scoring systems* have changed over the years, so it is a little unfair to draw too many conclusions by simply considering the sum of total points scored by each driver. In the following plot whe can see how points system have changed during the history of Formula One.
...
You can see the significant step in maximum points available in 2010 when the points changed from 10 points for a win to 25. 
It's clear how evaluating drivers on the basis of the total points would be a dishonest method towards the pilots prior to 2010.

A more fair approach is the following: recalculate the points awarded by drivers for each race in the history of Formula 1 always using the same scoring system. To normalize data we will use the currently used score system (but whithout the *fastest lap bonus point* rule, introduced in 2019). The points are awarded for the top 10 drivers in the following pattern: 1st gets 25, 2nd gets 18,15,12,10,8,6,4,2,1. The following two graphs show the ranking of the drivers by total points, before and after the data normalization.
...
```
As you can see the results still has Hamilton dominating the standings. However, Michael Schumacher is now much closer a few more drivers from outside of the recent years have started to be included. 
...
We need to set a requirement in order for a driver to be admitted in the comparison. If a driver won 1 race out of 1, starting in pole positions, he would be placed first in every classification, even if he is an irrelevant driver in the history of F1. The requirement to be included in the comparison is to have participated in at least 20 races (about 1 season).
...
Now that we have isolated the two main factors of variability between different F1 eras, we can define a more reasonable evaluation criterion based on the ratios of the metric illustrated above. The *performance* metric, which represents a driver's success and skills during their career, is calculated as follows:

$$performance = \frac{1}{4} * win \; rate+ \frac{1}{4}*podium \; rate + \frac{1}{4} *points \; rate+\frac{1}{4}*pole \; rate$$
Like before, only driver with at least 20 held races are considered in the 
...
Another factor which can be considered is the fact that car reliability improved over time, so modern driver have an advantage in their ratio statistics. Consider this example: driver *a* and driver *b* started both 100 races. Driver *a* is from the 50s and had 40 mechanical failures, so his real win potential is 60. Driver *b* is a recent *F1* driver, and had only 5 mechanical failurs, so his real win potential is 95. A fairer approach would take this issue into account, and for example, the *win rate* would be calculated in relation to completed races and not based on the races started. The graph below shows how old cars had a higher probability of mechanical failure.
...
We can clearly see how modern drivers have had, in proportion to the races started, much fewer retirements due to mechanical problems, therefore, they have an advantage in the calculation related to the metrics previously used for *performance* metric. We can fix this problem by introducing a performance multiplier for each driver, calcualted as follow:
...
### Conclusions on the analysis of drivers
...
```{r, out.width = "1440px",echo = !knit_relazione,fig.align='center'}
